...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `hey kak @${num.split('@')[0]} Yiyi Titip Gorengan Yah Btw Gausa Kembali Lagi ðŸ˜‹`
				let buffer = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buffer, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				wait: '[TUNGGU] YIYI sedang MemperosesðŸ¥Orang sabar Di Sayang Dajal!',
				asik: '[TUNGGU] YIYI sedang MemperosesðŸ¥Orang sabar Di Sayang Dajal!',
			...
			switch(command) {
				...
				if (!isUser) return reply(mess.only.userB)
			       ...
			       case 'valxlove':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*â•­â”€â”€â”€â”€*ã€Œ *ADMINBOT YIYI âœ¨* ã€\n*â”‚+wa.me/6285752164908*â•°â”€â”€â”€â”€â”€â”€*ã€Œ *YIYI* ã€*â”€â”€â”€â”€*\n\n*_JIKA INGIN MENJADI ADMIN  YIYI BOT_*\n*_Ketik /iklan_*' })
					break
					case 'premiumcek':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
...
conn.sendMessage(m.key, 'blabla', MessageType.audio, {mimetype: "audio/mp4", ptt: true})
		...
		case 'iri':
            ...
            case 'bodoh':
				...
				if (!isUser) return reply(mess.only.userB)
				if (isBanned) return reply(mess.only.benned)    
					teks = 'ðð‹ðŽð‚ðŠ ð‹ðˆð’ð“ :\n'
					for (let block of blocked) {
						teks += `â”£âž¢ @${block.split('@')[0]}\n`
					}
					teks += `ð“ð¨ð­ðšð¥ : ${blocked.length}`
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`â•­â”€â”€â”€â”€*ã€Œ *TOTAL USER BOT  YIYIðŸ‘‘* ã€\n\`\`\``
					no = 0
...
if (isBanned) return reply(mess.only.benned)    
          ...
          if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (!tex) return client.sendMessage(from, 'ðŒðšð¬ð®ð¤ðšð§ ð“ðžð¤ð¬/ð”ð«ð¥ ð˜ðšð§ð  ðˆð§ð ð¢ð§ ðƒð¢ ðð®ðšð­ ðð‘', text, {quoted: mek})
	...
	if (isBanned) return reply(mess.only.benned)    
			    ...
			    if (!isGroupAdmins) return reply(mess.only.admin)
				if (!isBotGroupAdmins) return reply(mess.only.Badmin)
				if (args.length < 1) return reply('ð­ðžð¤ð¬ð§ð²ðš ð¤ðšð¤')
					ppUrl = await client.getProfilePicture(from) // leave empty to get your own
...
if (isBanned) return reply(mess.only.benned)    
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isNsfw) return reply('âŒ *MODE NSFW BELUM DIAKTIFKAN* âŒ')
...
if (isBanned) return reply(mess.only.benned)    
 ...
 if (isBanned) return reply(mess.only.benned)    
				...
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					if (!isAnime) return reply('âŒ *Harus Mengaktifkan Mode Anime* âŒ')
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					if (!isNsfw) return reply('âŒ *Harus Mengaktifkan Mode Nsfw* âŒ')
 ...
 if (!isOwner) return reply(mess.only.ownerB)
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (!isUser) return reply(mess.only.userB)
					...
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args[0] === 'buka') {
					    reply(`ððžð«ð¢ð§ð­ðšð¡ ðð¢ð­ðžð«ð¢ð¦ðš, ð›ðžð«ð¡ðšð¬ð¢ð¥ ð¦ðžð§ð ð®ð›ðšð¡ ð ð«ð¨ð®ð© ð¬ðžð¦ð®ðš ð¨ð«ðšð§ð  ð›ð¢ð¬ðš ð¦ðžð§ð ð¢ð«ð¢ð¦ ð©ðžð¬ðšð§`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'tutup') {
						reply(`ððžð«ð¢ð§ð­ðšð¡ ðð¢ð­ðžð«ð¢ð¦ðš, ð›ðžð«ð¡ðšð¬ð¢ð¥ ð¦ðžð§ð ð®ð›ðšð¡ ð ð«ð¨ð®ð© ð¡ðšð§ð²ðš ðšðð¦ð¢ð§ ð²ðšð§ð  ð›ð¢ð¬ðš ð¦ðžð§ð ð¢ð«ð¢ð¦ ð©ðžð¬ðšð§`)
			...
			if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
					...
					mentions('â•”â•â•âœªã€˜ Mention All ã€™âœªâ•â•'+teks+'â•šâ•ã€˜  YIYI BOT ã€™', members_id, true)
					break
				case 'clearall':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
